...
const Menubar = () => {
  ...
  // update active menu whenever path changes
  useEffect(() => {
    const currentPath = pathname?.split("/")[1] || "home";
    setActiveMenu(currentPath);
    localStorage.setItem("menu", currentPath);
  }, [pathname]);
  ...
  return (
    <div className="mt-12">
      ...
      <nav
        ...
        className="fixed bottom-0 inset-x-0 m-auto mt-4 max-w-md w-full shadow-md px-6 py-2 z-50"
      >
        <div className="flex justify-between items-center">
          ...
          >
            <GoHomeFill
              className={`h-5 w-5 ${
                activeMenu === "home" ? "text-purple-700" : "text-gray-500"
              }`}
            />
            <span
              className={`text-[13px] ${
          ...
          >
            <FaSearch
              className={`h-5 w-5 ${
                activeMenu === "search" ? "text-purple-700" : "text-gray-500"
              }`}
            />
            <span
              className={`text-[13px] ${
