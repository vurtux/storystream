...
const FeatureRow: React.FC<FeatureRowProps> = ({ name, freeIcon, playIcon }) => {
  const freeIconColor = freeIcon === 'x' ? 'text-gray-400' : 'text-blue-200';
...
const SubscriptionPage: React.FC = () => {
  ...
  return (
    <div
      ...
      style={{
        background: 'linear-gradient(191.91deg, #6B0DFF -1.09%, #FF6B79 100%)',
      }}
    >
      ...
      <div className="flex-1 w-full max-w-2xl mx-auto">
        ...
        <div className="p-4 pt-6 text-center">
          ...
          <h1 className="text-2xl font-bold mb-1">
            Subscribe to <span className="text-yellow-300">storyStream</span>
          </h1>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {plans.map(({ price, period }, i) => (
              <div
                ...
                cursor-pointer transition-transform hover:scale-105 hover:bg-white/30"
              >
                <span className="text-lg font-bold block">{price}</span>
                <span className="text-xs text-white/90">{period}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Premium Features */}
        <div className="mt-6 p-6 pb-2 mx-auto bg-white/10 backdrop-blur-md rounded-xl">
          <h2 className="text-xl font-bold text-center mb-4">
            Premium features with <span className="text-yellow-300">storyStream</span>
          </h2>

          <div className="flex items-center mb-2 text-sm font-semibold border-b border-white/40 pb-2">
            <div className="flex-1 pr-4"></div>
            <div className="w-[20%] text-center font-bold text-gray-300">Free</div>
            <div className="w-[20%] text-center font-extrabold text-white">Pro</div>
          </div>
        ...
        {/* Offer Terms */}
        <div className="p-4 pt-6 text-white/90 text-left">
          <h3 className="text-sm font-bold mb-2">Promotional Offer Terms</h3>
          <ul className="list-disc pl-5 text-xs space-y-2">
            <li>
              Subscriptions at the offer price remain valid at renewal unless changed.
            </li>
            <li>
              Cancel anytime via Profile → Manage storyStream.
            </li>
      ...
      {/* Bottom CTA */}
      <div className="p-4 pt-6 sticky bottom-0 bg-white/10 backdrop-blur-md w-full text-center">
        <button
        ...
        <p className="text-[11px] mt-2 text-white/80 font-medium">
