...
const HomeClient = () => {
  ...
  fetchAndActivate(remoteConfig)
    .then(() => {
      ...
      const allValues = getAll(remoteConfig); // returns { [key: string]: Value }
      const result: Record<string, string> = {};

      Object.keys(allValues).forEach((key) => {
        result[key] = allValues[key].asString(); // convert each to string
      });

      console.log("✅ All Remote Config values:", result);
      setRemoteMessage(JSON.stringify(result, null, 2)); // optional: display all in UI
    })
  ...
  return (
    <div className="relative">
      ...
      {/* 🔹 Show Remote Config Value */}
      {remoteMessage && (
        <div className="text-center py-2 bg-gray-100 text-sm text-gray-700">
          {remoteMessage}
        </div>
      )}
