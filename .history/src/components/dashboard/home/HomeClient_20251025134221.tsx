...
const HomeClient = () => {
  ...
  // Initial setup - handle country and lang from URL
  useEffect(() => {
    const countryParam = searchParams.get("country");
    const langParam = searchParams.get("lang");

    // Save country to localStorage if present in URL
    if (countryParam) {
      localStorage.setItem("country", countryParam);
    }

    // Save language to localStorage if present in URL
    if (langParam) {
  ...
  useEffect(() => {
    ...
    if (!sid) {
      // Agar sid nahi hai to bhi home data fetch karo
      getHomeData();
      
      // Check for popup even without sid
    ...
    const validateUser = async () => {
      try {
        ...
        if (res.response.status) {
          ...
          // Check if there's a popup parameter
          const popupParam = searchParams.get("popup");
          ...
          if (popupParam === "1") {
            ...
            // Agar popup nahi hai to direct redirect
            router.push("/home");
          }
        ...
        // Error ke case mein bhi home data fetch karo
        getHomeData();
      }
    };
