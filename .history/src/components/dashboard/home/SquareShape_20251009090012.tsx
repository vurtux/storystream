...
const SquareShape = ({ data }: { data: SquareBlock }) => {
  ...
  const handleDetail = (
    ...
    router.push(
      `/home/podcast/${encodeURIComponent(conId)}/${slugify(
        conName || "unknown",
        { lower: true }
      )}`
    );
  ...
  const handleSeeAll = (item: SquareContent[]) => {
    ...
    router.push(
      `/home/see-all/${slugify(data.bkName || "unknown", { lower: true })}`
    );
  };
      ...
      {loading ? (
        ...
        >
          {[...Array(4)].map((_, idx) => (
            <div
              key={idx}
              className="min-w-[150px] h-[150px] rounded-lg bg-gray-300 animate-pulse"
            />
          ))}
        </div>
      ) : (
