...
export default function PodcastClient({ episode_id, title }: any) {
  ...
  const handleAudioChange = (temp: any) => {
    ...
    if (temp === "plus" && currentAudio < audioList.length - 1) {
      ...
      router.push(
        `/episode/${encodeURIComponent(audioList[currentAudio + 1])}/${slugify(
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const ep =
          result?.response?.podcast?.podcast_episode_details?.length > 0
            ? result?.response?.podcast?.podcast_episode_details[0]
            : {};
        setEpisodeData(ep);
          ...
          {[0.5, 1, 1.5, 2].map((rate) => (
            <button
              ...
              className={`py-2 px-4 rounded-md text-white ${
                playbackRate === rate ? "bg-purple-600" : "bg-gray-400"
              }`}
            >
              {rate}x
            </button>
      ...
      >
        <div className="bg-white text-center w-full px-6 py-4">
          ...
          <p className="text-gray-600 my-4">
            Subscribe now to enjoy unlimited access to StoryStream for R5/day on your Vodacom Account/Airtime. First day is free.
          </p>
          <button
