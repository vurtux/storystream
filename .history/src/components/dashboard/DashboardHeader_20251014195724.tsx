...
const HeaderSlider = ({ data }: { data: SpotlightBlock }) => {
  ...
  const settings = {
    dots: slideCount > 1,
    arrows: false,
    infinite: true, // Always scroll infinitely
    centerMode: true, // Always centered
    centerPadding: slideCount === 1 ? '0px' : slideCount === 2 ? '5%' : '10%',
    ...
    responsive: [
      {
        ...
        settings: {
          slidesToShow: 1,
          centerPadding: slideCount === 1 ? '0px' : '10%',
        },
      },
    ],
    appendDots: (dots: any) => (
