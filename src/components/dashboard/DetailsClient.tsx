...
import { trackPageView, buildProductData } from "../../lib/tealiumTracking";
...
const DetailsClient = ({ conId, title }: any) => {
  ...
  // Track podcast view when data loads
  useEffect(() => {
    if (podcastData && !loading) {
      try {
        const raw = localStorage.getItem("loginData");
        let userId = "guest";
        ...
        if (raw) {
          try {
            const data = JSON.parse(raw);
            userId = data?.profile?.userId || "guest";
          } catch (e) {
        ...
        const productData = buildProductData({
          productId: podcastData.podcast_id.toString(),
          productName: podcastData.title,
          productPrice: podcastData.is_billable === 1 ? "99.99" : "0",
          productCategory: podcastData.ptype === "book" ? "Audiobook" : "Podcast",
          productSku: `POD-${podcastData.podcast_id}`,
          productType: podcastData.is_billable === 1 ? "paid" : "free",
        });
        ...
        console.log("📊 Tracked podcast view:", podcastData.title);
      } catch (error) {
  ...
  const handleEpisode = (item: PodcastEpisodeDetail, index: number) => {
    try {
      ...
      // Track episode play event
      try {
        const productData = buildProductData({
          productId: item.episode_id.toString(),
          productName: item.title,
          productPrice: item.is_billable === 1 ? "9.99" : "0",
          productCategory: "Episode",
          productSku: `EP-${item.episode_id}`,
          productType: item.is_billable === 1 ? "paid" : "free",
        });
        ...
        console.log("📊 Tracked episode play:", item.title);
      } catch (trackError) {
        console.log("Error tracking episode play:", trackError);
      }
  ...
  const handlePlayButton = () => {
    if (episodeData && episodeData.length > 0) {
      ...
      try {
        ...
        const productData = buildProductData({
          productId: firstEpisode.episode_id.toString(),
          productName: firstEpisode.title,
          productPrice: firstEpisode.is_billable === 1 ? "9.99" : "0",
          productCategory: "Episode",
          productSku: `EP-${firstEpisode.episode_id}`,
          productType: firstEpisode.is_billable === 1 ? "paid" : "free",
        });
        ...
        console.log("📊 Tracked first episode play:", firstEpisode.title);
      } catch (trackError) {
        console.log("Error tracking play:", trackError);
      }

      handleEpisode(firstEpisode, 0);
    }
  };
    ...
    try {
      ...
      if (!isVip) {
        ...
        try {
          ...
          console.log("📊 Tracked episode download:", item.title);
        } catch (trackError) {
          console.log("Error tracking download:", trackError);
        }
