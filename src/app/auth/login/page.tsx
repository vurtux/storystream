  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      try {
        ...
        localStorage.setItem("mobile", finalMobileNo);
      } catch (storageError) {
        console.error("localStorage error:", storageError);
      }

      // Check MDN profile first
      const payloadMDN = { mobileNo: finalMobileNo, isdCode };
      ...
      if (!mdnRes?.response?.status) {
        ...
        if (isSubscribed === "true" && storedPlan) {
          const linkWithMobile = `${storedPlan}&msisdn=${finalMobileNo}`;
          window.location.href = linkWithMobile;
          return;
        } else {
          // showError("Please subscribe to continue");
          const redirectUrl = finalMobileNo?.trim()
            ? `/subscribe?fromLogin=true&msisdn=${finalMobileNo}`
            : "/subscribe?fromLogin=true";
        ...
        <div className="absolute bottom-[100px] left-0 right-0 z-10 px-6 pb-12 pt-6 bg-gradient-to-t from-white via-white to-transparent">
          ...
          <div className="text-center mt-4">
            <Link
              href={
                mobileNo?.trim()
                  ? `/subscribe?fromLogin=true&msisdn=${mobileNo.slice(-9)}`
                  : "/subscribe?fromLogin=true"
              }
